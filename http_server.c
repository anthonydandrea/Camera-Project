...
void printTime(struct client* client);
  ...
  while(is_running(s)){
    ...
    camera_open();
      ...
      case 'g':
           ...
           //setup_packet(client,frame_sz);
    //  client_send_frame(&p, 86);
     // camera_get_frame(USE_CAMERA);
      camera_open();
...
void printTime(struct client* client){
  frame *fr = fr = camera_get_frame(client->cam);
/*  #ifdef USE_CAMERA
  unsigned long timme = get_frame_timestamp(fr);
  printf("The current image has a timestamp %lu\n",timme );
  #endif
  */
  unsigned long long timme = get_frame_timestamp(fr);
  //unsigned long timme =  capture_frame_timestamp(&fr);
  int size = 64;
  char buffer[20];
  snprintf(buffer,size,"%lld\n",timme);
  size = strlen(buffer);
  printf("The size of the long is %d\n",size);
  printf("The buffer consists of: %s\n", buffer);
  write_n(client->connfd,buffer,size);
  printf("The current image has a timestamp %lld\n", timme );
}
